			...
			<div
				...
				<table class="table table-hover table-bordered table-striped">
					...
					<c:forEach var="payment" items="${allPayment}" varStatus="i">
						<tr>
							<c:if test="${ (i.count) == 1 }">
								<td rowspan="${num}">${faculty.facultyID}</td>
								<td rowspan="${num}">${faculty.employee.user.firstName} ${faculty.employee.user.middleName} ${faculty.employee.user.lastName}</td>
							</c:if>
							<td>${payment.id}</td>
							<td>${payment.month}</td>
							<td>${payment.year}</td>
							...
							<c:if test="${curRole=='admin'}">
								<td><a href="/admin/update/payment/faculty/${payment.id}/">Edit</a></td>							
								<td><a href="/admin/delete/payment/faculty/${payment.id}/">Delete</a></td>
							</c:if>
						</tr>					
					</c:forEach>
