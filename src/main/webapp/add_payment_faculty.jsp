		...
		<div class="row">
			<div class="col">
				...
				<form:form action="" method="post" class="needs-validation m-3"
					...
					<div class="form-group">
						<c:set var="facultyErrors"><form:errors path="employee.employeeID" /></c:set>
						<form:label path="employee.employeeID" >Faculty:</form:label>
			            <form:select path="employee.employeeID" class="custom-select ${not empty facultyErrors ? 'is-invalid' : '' }" required="true">
			            	<option selected disabled value="">Choose...</option>
							<c:forEach var="faculty" items="${allFaculty}" varStatus="i">
								<form:option path="employee.employeeID" value="${faculty.employee.employeeID}" >${faculty.employee.employeeID}: ${faculty.employee.user.firstName} ${faculty.employee.user.middleName} ${faculty.employee.user.lastName} (Salary: ${faculty.employee.salary})</form:option>
							</c:forEach>
						</form:select>
